...
scope: root
covers: [build, artifacts, out-of-tree, tooling, docker, tests, worktree]
---
...
- Configure build systems (CMake, npm/webpack, cargo, tox, etc.) to point their output directory
  outside the repo root — e.g. a sibling directory or a path mounted from outside the tree in the
  container setup (see [[container-only-development]]) — rather than relying solely on `.gitignore`.
  `.gitignore` protects commits; it does not protect a git worktree checkout from getting polluted.
- This matters especially because work happens in git worktrees (see [[git-worktree-oneflow]]):
  in-tree artifacts either get duplicated per-worktree or leak across them, both of which are wrong.
