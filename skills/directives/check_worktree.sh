...
# isolated git worktree. Run before any mode that writes to directive docs.
...
if [[ "$current_toplevel" == "$primary_toplevel" ]]; then
  echo "error: refusing to mutate directive docs in the primary checkout ($primary_toplevel)." >&2
